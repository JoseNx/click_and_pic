...
class Usuario {
    ...
    public static function Usuario_Nuevo($loginRegistro, $contraseñaRegistroCod, $nombreRegistro, $apellidoRegistro, $correoRegistro) {
        ...
        if ($comprobarUsuario->rowCount() == 0) {
            $conexion->beginTransaction();
            $sql = "INSERT INTO `usuario`(`login`, `nombre`, `apellido`, `correo`, `contraseña`, `rol`, `foto_perfil`) VALUES ('$loginRegistro','$nombreRegistro', '$apellidoRegistro', '$correoRegistro', '$contraseñaRegistroCod', 'normal', '')";
