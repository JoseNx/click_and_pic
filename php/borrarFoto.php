...
//Ejecucion
if (isset($borrar)) {
    foreach ($_POST['publicacionEliminar'] as $publicacionDato) {
        $consultaDevolver = "DELETE FROM `publicacion` WHERE cod_publi='$publicacionDato'";
        $conexion->query($consultaDevolver);
    }
    header("Location: borrarFoto.php");
...
<html lang = "Es-es">
    ...
    <head>
        ...
        <link rel = "stylesheet" type = "text/css" href = "../css/borrarFoto.css" />
                ...
                <table  id="tablaListaControl">
                    ...
                    <tbody>
                        <?php foreach ($listaPublicacion as $publicacionDato) {
                            ?>
                            <tr>
                                <td><?php echo $publicacionDato['titulo'] ?></td>
                                <td><img width="100" src="data:<?php echo $publicacionDato['tipo']; ?>;base64,<?php echo base64_encode($publicacionDato['foto_publicacion']); ?>"></td>
                                <td><input type="checkbox" name="publicacionEliminar[]" value="<?php echo $publicacionDato['cod_publi'] ?>"/></td>
                            </tr>
                        </tbody>
